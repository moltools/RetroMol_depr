...
#     def test_linearization_migrastatin(self):
#         mol = Molecule(r"C[C@@H]1/C=C(\[C@H](OC(=O)/C=C/CC/C=C/[C@@H]([C@H]1O)OC)[C@H](C)C(=O)CCCC2CC(=O)NC(=O)C2)/C")
#         results = linearize(mol)
#         expected = Molecule(r"C/C(=C/[C@@H](C)[C@H](O)[C@@H](O)/C=C/CC/C=C/C(=O)O)[C@H](O)[C@H](C)C(=O)CCCC1CC(=O)NC(=O)C1")
#         self.assertEqual(len(results), 1)
#         self.assertTrue(results[0] == expected)

#     def test_sequencing_migrastatin(self):
#         mol = Molecule(r"C/C(=C/[C@@H](C)[C@H](O)[C@@H](O)/C=C/CC/C=C/C(=O)O)[C@H](O)[C@H](C)C(=O)CCCC1CC(=O)NC(=O)C1")
#         result = [str(u) for u in sequence(mol)]
#         expected = ["D1", "A2", "B2", "C2", "B5", "C1", "D1", "C1"]
#         self.assertEqual(result, expected)
...
#     def test_linearization_penicillin_G(self):
#         mol = Molecule(r"CC1([C@@H](N2[C@H](S1)[C@@H](C2=O)NC(=O)CC3=CC=CC=C3)C(=O)O)C")
#         results = linearize(mol)
#         expected = Molecule(r"CC(C)[C@@H](NC(=O)[C@H]([CH]S)NC(=O)Cc1ccccc1)C(=O)O") 
#         self.assertEqual(len(results), 1)
#         self.assertTrue(results[0] == expected)

#     def test_sequencing_penicillin_G(self):
#         mol = Molecule(r"CC(C)[C@@H](NC(=O)[C@H]([CH]S)NC(=O)Cc1ccccc1)C(=O)O") 
#         result = [str(u) for u in sequence(mol)]
#         expected = ["AlphaAminoAcid", "Val"]
#         self.assertEqual(result, expected)
