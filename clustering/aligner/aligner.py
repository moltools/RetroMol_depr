...
class ModuleSequence:
    def __init__(
        ...
        self.name = name
        if any([isinstance(x, str) for x in module_sequence]):
    ...
    def parse(
        ...
        def get_polyketide_subunit(module: str, gap: Optional[str] = '-'):
            ...
            x = module
            # Translate module:
            if x[0].isalpha() and x[1:].isdigit() and x[0] == "A": module = "A"
            elif x[0].isalpha() and x[1:].isdigit() and x[0] == "B": module = "B"
            elif x[0].isalpha() and x[1:].isdigit() and x[0] == "C": module = "C"
            elif x[0].isalpha() and x[1:].isdigit() and x[0] == "D": module = "D"
            elif x in ["Arg", "Gln", "Aad", "Asp", "aAsp/Asp", "Glu", "aGlu/Glu", "Asn", "OH-Asn", "MeO-Glu", "bOMe-Asp", "Dab", "Ser", "Ph-Ser", "Ac-Ser"] or "Orn" in x: module = "AA_1"
            elif x in ["Val", "bOH-Val", "Aib", "Leu", "3OH-Leu", "Ile/aIle", "NFo-Leu", "NAc-Leu"]: module = "AA_2"
            elif x in ["Tyr", "bOH-Tyr", "Phe", "Trp", "OH-Trp", "dh-Trp", "Hpg"]: module = "AA_3"
            elif x in ["aThr/Thr", "Ala", "Abu", "dhAbu", "bAla", "Dpr"]: module = "AA_4"
            elif x in ["Pro", "NFo-Pro", "aPro/Pro", "NMe-Hpr", "Hpr"]: module = "AA_5"
            elif x in ["Cys", "NAc-Cys", "Met"]: module = "AA_6"
            elif x in ["Gly"]: module = "AA_7"
            else: 
                raise ValueError(f"Invalid compound {x}")

            module = PKModule[module]
            module.original_name = x

            return module

        # def starting(char) -> bool:
        #     return char.isalpha() or char == PKModule.GAP.display_name()

        module_list = []
        tag = None 
        for module in module_sequence_string:
            module_list.append(
...
class MultipleSequenceAlignment:
    def __init__(self, records: List[Record], gap_cost: int, gap_end_cost: int) -> None:
    ...
    def _align(self) -> None:
        ...
        """
        def all_pairwise_scores(seqs1: List[ModuleSequence], seqs2: List[ModuleSequence]) -> List[PKModule]:
        ...
        if len(self._records) == 0: msa = {0: [ModuleSequence('', '')]}  # Return empty alignment
        elif len(self._records) == 1: msa = {0: self._records}  # Return seq if single
        else:
            ...
            for seq in self.msa[keys[0]]:
                ...
                for m in seq._seq:
                    if m[0] == PKModule.GAP:
                        ...
                        seq_str.append(m[0].original_name)
