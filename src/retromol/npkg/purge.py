...
def purge_retrosynthesis_data(
    ...
    if only_calculated:
        query = (
            "MATCH (n:MotifCode {calculated: true}) WITH n LIMIT $limit "
            "MATCH (n:Motif {calculated: true}) WITH n LIMIT $limit "
            "DETACH DELETE n "
            "RETURN count(n) AS deleted_count"
        )
    else:
        query = (
            "MATCH (n:MotifCode) WITH n LIMIT $limit "
            "MATCH (n:Motif) WITH n LIMIT $limit "
            "DETACH DELETE n "
            "RETURN count(n) AS deleted_count"
        )

    # Purge the retrosynthesis data.
    while True:
        result = conn.query(query, {"limit": 1000})
        deleted_count = result[0]["deleted_count"]
        if deleted_count == 0:
