...
class PolyketideMotif(Motif):
    ...
    def from_dict(cls, data: ty.Dict[str, ty.Any]) -> "PolyketideMotif":
        ...
        # Determine pk_type.
        if len(accesory_domains) == 0: pk_type = PolyketideType.A
        elif len(accesory_domains) == 1 and "KR" in accesory_domains: pk_type = PolyketideType.B
        elif len(accesory_domains) == 2 and "KR" in accesory_domains and "DH" in accesory_domains: pk_type = PolyketideType.C
        elif len(accesory_domains) == 3 and "KR" in accesory_domains and "DH" in accesory_domains and "ER" in accesory_domains: pk_type = PolyketideType.D
        else: pk_type = PolyketideType.Undefined
...
class PeptideMotif(Motif):
    ...
    def from_dict(cls, data: ty.Dict[str, ty.Any]) -> "PeptideMotif":
        ...
        if classification == "small non-hydrophobic": peptide_type = PeptideType.SmallNonHydrophobic
        elif classification == "small hydrophobic": peptide_type = PeptideType.SmallHydrophobic
        elif classification == "cyclic aliphatic": peptide_type = PeptideType.CyclicAliphatic
        elif classification == "cysteine-like": peptide_type = PeptideType.CysteineLike
        elif classification == "polar and charged": peptide_type = PeptideType.PolarAndCharged
        elif classification == "tiny": peptide_type = PeptideType.Tiny
        elif classification == "bulky": peptide_type = PeptideType.Bulky
        else: peptide_type = PeptideType.Undefined
...
# TODO: make scoring matrix that takes into account for ...
# polyketides -- domain combination (same or not the same), decoration (same or not the same), and if one or both are None
# peptides -- classification, and if one or both are none
# polyketide/peptide -- should be penalized
def score_motif_similarity(motif1: Motif, motif2: Motif) -> int:
    ...
    if (
        isinstance(motif1, PolyketideMotif) and isinstance(motif2, PeptideMotif) or 
        isinstance(motif1, PeptideMotif) and isinstance(motif2, PolyketideMotif)
    ):
        return -1
...
class MultipleSequenceAlignment:
    ...
    def _align(self) -> None:
        """
        Based on: 'Progressive Sequence Alignment as a Prerequisite to Correct Phylogenetic Trees' by Feng and Doolittle, 1987
        """
