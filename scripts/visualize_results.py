#!/usr/bin/env python3
"""
Description:    Evaluate RetroMol results with scores histogram.
Usage:          python3 evaluate_results.py -i <path/to/results/dir> -o <path/to/output/dir>
...
def main() -> None:
    ...
    for i, filepath in enumerate(filepaths):
        ...
        padding = str(len(str(len(filepaths))))
        print(f"{i}".zfill(padding) + f"/{len(filepaths)}", end="\r")
    ...
    plt.savefig(os.path.join(args.output, "histogram_scores.png"), dpi=300)   
    ...
    plt.savefig(os.path.join(args.output, "histogram_scores_as_percentages.png"), dpi=300)
    ...
    plt.savefig(os.path.join(args.output, "histogram_number_of_atoms.png"), dpi=300)
